...
在其之上新增本產學合作專案的展示與文件，**不改動上游密碼學實作**。
...
公司端執行 `docker compose up` 即啟動全套，資料完全留在內網：
...
│  │  IGenValInvoker │:30000│  (ML-KEM / ML-DSA)        │ │
│  │  前端 SPA        │      └──────────────────────────┘ │
│  └────────────────┘                                    │
└──────────────────────────────────────────────────────┘
        ▲ 瀏覽器（公司非技術人員）
...
| 層 | 選擇 |
|----|------|
| 後端 | ASP.NET Core 8 Web API（複用 `gen-val` 的 Generation/Common 函式庫）|
| 運算 | 既有 Orleans Silo（不改動，當作 crypto 後端）|
| 前端 | React + Vite（或 Blazor）|
| 封裝 | Docker Compose |

詳見 [專案計劃書](project-docs/planning/PROJECT_PLAN.md)。
...
| 後端 API（`IGenValInvoker` 程式化呼叫）| ⬜ 規劃中（S0–S2）|
| 前端 SPA + 非專家 UX | ⬜ 規劃中（S3–S4）|
| Docker 封裝 + 交付 | ⬜ 規劃中（S5）|

> ⚠️ 目前 demo 的 IUT 腳本為 **Mock Harness**：它讀取 server 隨機產生的 `expectedResults.json`
> 來自動作答，用以展示完整 ACVP 生命週期（出題→作答→批改→Disposition），**不執行真實 PQC 運算**。
> 真實對接時，這一步會換成公司用自家模組產生 `responses.json`。
...
所有自行新增的文件集中於 [`project-docs/`](project-docs/)：
...
- 上游原始碼（`gen-val/`、`docs/`、`_config/`）來自 NIST [`usnistgov/ACVP-Server`](https://github.com/usnistgov/ACVP-Server)，
  其原始 README 與建置/測試說明請見上游 repo 與其 [Wiki](https://github.com/usnistgov/ACVP-Server/wiki)。
- 授權沿用上游 NIST 條款，見上游 repo 的[授權說明](https://github.com/usnistgov/ACVP-Server)。
- 本 fork 新增內容（`project-docs/`、`fips-203-204-demo/`）為本專案產學合作成果。
</content>
