#!/usr/bin/env python3
"""ML-DSA (FIPS 204) response harness.
...
Usage:  python3 harness_mldsa.py [path/to/prompt.json]
"""
...
def invoke_module(mode: str, group: dict[str, Any], test: dict[str, Any]) -> dict[str, Any]:
    """Answer ONE test case with YOUR ML-DSA module.

    Required return value per mode (all binary values are lowercase hex):
      keyGen  -> {"pk": "<hex>", "sk": "<hex>"}
      sigGen  -> {"signature": "<hex>"}
      sigVer  -> {"testPassed": True | False}

    Inputs: ``group`` carries e.g. parameterSet, deterministic, preHash and
    signatureInterface; ``test`` carries the per-case prompt fields such as
    message / mu, rnd, context and hashAlg (see field-mapping.md).
    """
    # ======================= FILL IN THIS LINE =======================
    raise NotImplementedError("call your ML-DSA module here")
    # =================================================================
